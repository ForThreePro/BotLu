...
let handler = async (m, { conn, groupMetadata }) => {
  ...
  if (participantes.length < 4) {
    return m.reply('😺 *BOT LU - CUPIDO* 😺\n\nAy nooo me faltan gatitos 🥺👉👈\nNecesito mínimo 4 en el grupo para shippear 2 parejitas 💘')
  }

  // Mezclar y sacar 4 personas random
  let mezclados = participantes.sort(() => Math.random() * 2 - 1)
  ...
  // Armar el mensaje
  let texto = `😺 *BOT LU - MODO CUPIDO* 😺\n\n`
  texto += `✨ *Activando chismecito dulce y vibras gatunas* ✨\n\n`
  ...
  texto += `😺 Hecho con mucho cariño por *Bot Lu* 😺`
